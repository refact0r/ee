 ...
 * </p>
 * <p>
 * Then use the corresponding "ArithmeticDecompress" application to recreate the
 * original input file.
 * </p>
 * <p>
 * Note that the application uses an alphabet of 257 symbols - 256 symbols for
 * the byte
 * values and 1 symbol for the EOF marker. The compressed file format starts
 * with a list
...
public class ArithmeticCompress {
	...
	// // Read input file again, compress with arithmetic coding, and write output
	// file
	// try (InputStream in = new BufferedInputStream(new
	// FileInputStream(inputFile));
	// BitOutputStream out = new BitOutputStream(new BufferedOutputStream(new
	...
	public static void main(String[] args) throws IOException {
		...
		for (File inputFile : inputFolder.listFiles()) {
			// File outputFile = new File(outputFolder, inputFile.getName() + "-java-ari");
